...
@section('content_header')
  <h1>Laporan Bobot Kriteria</h1>
  <h6 class="text-secondary">Cetak Laporan Bobot Kriteria</h6>
@stop
@section('content')
<section class="mb-5">
  @include('admin/alert')
      ...
      <form class="form-horizontal" action="{{ url('admin/laporan/cetak') }}" method="post">
        ...
        <div class="form">
          <div class="form-group row">
            <div class="col-md-12">
              <input type="hidden" name="id_laporan" value="2">
              <label for="date" class="control-label">Tanggal / Kode</label>
              <select class="form-control" name="alternative_code">
                @foreach($data as $d)
                <option value="{{ $d->id_analisa_kriteria }}">{{ $d->date }} - {{ $d->id_analisa_kriteria }}</option>
                @endforeach
              </select>
            </div>
